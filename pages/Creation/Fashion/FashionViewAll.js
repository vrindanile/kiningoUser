...
// const FashionViewAll = (props, route) => {
...
//         console.log('art search has beeen called');
//         setLoading(true)
...
//             fUrl = fUrl + ru + urls
//         }
//         console.log('my urln home page for search---------->', fUrl)


//         const { responseJson, err } = await requestGetApi(fUrl, '', 'GET', User.token)
//         setLoading(false)
//         console.log('the res Home searcg frrrr==>>', responseJson)
//         if (responseJson.body.total > 0) {
//             console.log('the res after sucess of search cooking', responseJson)
//             generateThumb(responseJson.body.articles)
//         } else {
//             setCategorydata([]);
//         }
...
import { baseUrl, shop_eat_cart, user_payment_method, shop_eat_orders, shop_eat_cart_book_dining, shop_eat_cart_book_table, shop_eat_cart_id, shop_eat_business_id, shop_eat_menu_userid, requestPostApi, requestGetApi, shop_eat, connect_people_create_post, connect_people_newPost, art_getCollection, requestPostApiMedia, art_PostCollection, postApiWithToken2, art_HomePage, art_likeDislike, get_suggestion, creation_home, creation_searchHome, creation_startup, invention_home, getNewsAPI, getAPI, creation_Fashion } from '../../../WebApi/Service'
...
const FashionViewAll = (props, route) => {
    ...
    const getFilterStatement = async (pageNo = 1, search = '') => {
        try {
            ...
            if (status) {
                if (pageNo === 1) {
                    ...
                    if (response.body.total > 0) {
                        console.log('Data after successful search:', response.articles);
                    ...
                    console.log('Reached page other than 1');
                    generateThumb([...articleData, ...response.data]);
                    setPage(newpage); // Ensure that `newpage` is correctly defined and updated
                }
            }
    ...
    const searchFilter = async text => {
        ...
        if (text.text.trim() === '') {
            await getFilterStatement(1,);
        } else {
            await getFilterStatement(1, text.text);
        }
    ...
    const handleLoadMore = async () => {
        ...
        if (isFilterApplied) {
            await getFilterStatement(pageNumber,);
            pageNumber += 1;
        } else {
            await getStatement();
        }
    ...
    return (
        <SafeAreaView scrollEnabled={scrollEnabled} style={{ height: '100%', backgroundColor: '#F8F8F8' }}>
            ...
            <View style={{
                ...
                <View style={{ width: dimensions.SCREEN_WIDTH * 0.9, alignSelf: 'center', height: dimensions.SCREEN_HEIGHT * 0.78, }}>
                    {categoryData.length === 0 ? (
                        <>
