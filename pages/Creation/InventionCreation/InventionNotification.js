...
//             console.log('my notifications art', responseJson.body.data);
...
//                             { console.log(item, 'itobject_idem.') }
//                             return (
//                                 <TouchableOpacity style={{ width: '95%', alignSelf: 'center', backgroundColor: '#fff', borderRadius: 5, marginVertical: 10 }} onPress={() => {
//                                     { item.object_id != null ? props.navigation.navigate('CookingPost', { id: item.object_id, }) : props.navigation.navigate('CookingPost', { id: 68, }) }
...
const InventionNotifications = (props) => {
    const dispatch = useDispatch();
    ...
    const getAllNotificationsList = async () => {
        ...
        const { responseJson, err } = await requestGetApi(creation_getNotifications + 59, '', 'GET', User.token)
        setLoading(false)
        console.log('the res in_cart notification_list ==>>', responseJson)
        if (responseJson.headers.success == 1) {
            // var counts = 0
    ...
    const ClearNotification = async () => {
        setLoading(true)
        const { responseJson, err } = await requestPostApi(creation_deletenoti + 59, '', 'DELETE', User.token)
        setLoading(false)
        console.log('the res of cler notification', responseJson)
        if (responseJson.headers.success == 1) {
            console.log('the res of cler notification from creation', responseJson)
    ...
    const statusNotification = async (item) => {
        ...
        const { responseJson, err } = await requestPostApi(notification_status + 59, data, 'PUT', User.token)
        setLoading(false)
        ...
        <SafeAreaView style={styles.container}>
            <View style={{}}>
                <HomeHeader height={100} extraStyle={{ paddingtop: 10, paddingBottom: 25 }} paddingHorizontal={15} borderBottomLeftRadius={20} borderBottomRightRadius={20} backgroundColor='#FFC40C'
                    ...
                    press2={() => { }} title2={'Notification'} fontWeight={'500'} img2height={20} color={'#fff'}
                ...
                <View style={{ width: '100%', alignSelf: 'center', marginTop: 10, paddingHorizontal: 10, }}>
                    ...
                    {notificationData.length != 0 ?
                                ...
                                <TouchableOpacity style={{ width: '95%', alignSelf: 'center', backgroundColor: '#fff', borderRadius: 5, marginVertical: 10 }} onPress={() => {

                                    {
                                        item.article_id && item.object_id
                                            != undefined ? props.navigation.navigate('InventionPost', { id: item.article_id, }) : Toast.show({ text1: 'Article not Found' })
                                    }
                        ...
                        <View style={{ alignSelf: 'center', width: '90%' }}>
                            ...
                            <TouchableOpacity style={{ width: '100%', alignSelf: 'center', marginTop: 25 }} onPress={() => { props.navigation.navigate('InventionHome') }}>
                                <MyButtons title="Go To Home" height={35} width={'100%'} borderRadius={5} alignSelf="center"
                                    press={() => {
...
export default InventionNotifications;
