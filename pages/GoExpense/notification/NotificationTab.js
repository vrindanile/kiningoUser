...
const NotificationTab = ({ items, viewNotificationClick, handelRedirection }) => {
    // change date formate
    ...
    const handel = async (item) => {
        console.log('from the notification ab===>>', item);
        {
            item?.message.includes("expense exceeded the budget") ? navigation.navigate('ViewExpanses', {
                id: item.RedirectId
            }) : item?.message.includes("group") ? navigation.navigate('SplitDetail', {
                id: item.RedirectId
            }) : null
    ...
    return (
        <>
            {console.log('my notification tab----->>>', items.status === 1)}
            {items.status === 1 ? (
                <TouchableOpacity
